...
class KNN:
            ...
            def VoisinKNN(self,instanceTest , DataSet , k):
                ...
                for x in range (k):
                    ...
                    #Indice contient Element et l'indice de l'element
                    IndiceDist.append(distance[x][2])
                print("Les indices utilisés = "+format(IndiceDist))   
                return voisins
...
def graph_data(distance,step):
    ...
    steps.append(step)
############################################### 
def graph_making(distances,seconds):

    plt.plot(seconds,distances)
    plt.xlabel('steps')
    plt.ylabel('Distance')
...
def run_robot(robot,bool):
    ...
    while robot.step(timestep) != -1:
        ...
        if left_val > 0.9 :
            if middle_val<=0.9:
                verification="Sharp-Right-Turn"
            else:
                verification="Slight-Left-Turn"
        else:
            if middle_val<=0.9:
                verification="Sharp-Right-Turn"
            else:
                if left_val<0.55:
                    verification="Slight-Right-Turn"
                else:
                    verification="Move-Forward"
                

        if action != verification :
            instance_correct=[middle_val,left_val,verification]
            if (left_val !=1 and middle_val !=1):
                dataset = KNN.ADDToList(Obj1, dataset ,instance_correct)
        
        print("capteur milieu : "+format(middle_val)+"      capteur gauche : "+format(left_val))
        print("action prédite : "+format(action)+"      action corrigée : "+format(verification))
        print("**********************************************************"*3)
